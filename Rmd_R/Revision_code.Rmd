...
# Save (optional)
ggsave("TATA_hist_AS.pdf", p_hist_AS, width = 10, height = 8, units = "cm")
...
# -----------------------------
files <- c(
  Q1 = "path/to/mnase_q1.tsv",
  Q2 = "path/to/mnase_q2.tsv",
  Q3 = "path/to/mnase_q3.tsv",
  Q4 = "path/to/mnase_q4.tsv"
)
...
ggsave(p1, filename = "FFT_WT.pdf", height=7.5, width = 10, units = "cm")
```
...
tsv_files <- c(
  Q1 = "path/to/Q1_gene_signal.tsv",
  Q2 = "path/to/Q2_gene_signal.tsv",
  Q3 = "path/to/Q3_gene_signal.tsv",
  Q4 = "path/to/Q4_gene_signal.tsv"
)
...
# Save (optional)
ggsave("H2B_boxplot.pdf", p2, width = 12, height = 8, units = "cm")
...
fa_this <- "path/to/genotype_antisense_upstream.fasta"
fa_pcgs <- "path/to/spombe_pcg_upstream.fasta"
fa_rand <- "path/to/random_500bp.fasta"
...
# Save (optional)
ggsave("GC_barplot.pdf", p4, width = 8, height = 8, units = "cm")
```
...
# Save (optional)
ggsave("TATA_barplot.pdf", p3, width = 8, height = 8, units = "cm")
...
# Save (optional)
ggsave("TATA_hist_PCG.pdf", p_hist_PCG, width = 10, height = 8, units = "cm")
```
...
tomtom_results <- read_tsv("path/to/tomtom_test.tsv", col_types = cols())
...
ggsave("TF_heat.pdf", plot = TF_heat, width = 6, height = 4)
```


Figure 7C
```{r}
chipseq.all <- read.table("path/to/myc_scaled.tab", header = TRUE, sep = "\t", skip = 2)
...
ggsave(p14, filename = "myc.pdf", height=7.5, width = 10, units = "cm")
```
...
inside <- fread("/path/to/totals_in_genes.tsv", header = FALSE, col.names = c("sample","sum_in_genes"))
genome <- fread("/path/to/totals_genome.tsv", header = FALSE, col.names = c("sample","sum_total"))
...
fwrite(res, "gene_body_fraction.tsv", sep = "\t")
...
ggsave(p, filename = "myc_fraction.pdf", height=7.5, width = 7.5, units = "cm")
